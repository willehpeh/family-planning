...
describe('MarkItemAsDoneCommand', () => {
  ...
  it('should mark the item as done', async () => {
    ...
    const listSnapshot = inMemoryTodoListRepository.listSnapshots().find(list => list.id() === TEST_TODO_LIST_ID.value());
    const itemSnapshot = listSnapshot?.items().find(item => item.id() === TEST_TODO_LIST_ITEM_ID.value());
