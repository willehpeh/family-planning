...
import { UserCreatedEvent } from '@family-planning/domain';
...
describe('CreateNewUserCommand', () => {
  ...
  describe('UserCreatedEvent raised', () => {
    ...
    let raisedEvent: UserCreatedEvent;

    beforeEach(() => {
      raisedEvent = fakeEventBus.events[0] as UserCreatedEvent;
    });

    it('should raise a UserCreatedEvent', async () => {
      expect(raisedEvent).toBeInstanceOf(UserCreatedEvent);
    });
